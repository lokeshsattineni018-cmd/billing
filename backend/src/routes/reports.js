...
router.get('/sales', protect, restrictTo('owner', 'admin', 'staff'), async (req, res) => {
  try {
    ...
    let waMessage = `VIJAYA DURGA SEA FOODS\nSALES ANALYTICS REPORT (${rangeLabel})\n${dateStr}\n`;
    waMessage += `━━━━━━━━━━━━━━━━━━━\n`;
    ...
    waMessage += `Generated by Vijaya Durga Sea Foods Billing Suite`;
  ...
  try {
    ...
    const customers = rawOutstanding.map((c) => {
      const reminderMsg = `VIJAYA DURGA SEA FOODS\nPAYMENT REMINDER\n\nDear ${c.companyName},\nThis is a gentle reminder regarding pending dues for your account.\n\n` +
        ...
        `Thank you,\nVijaya Durga Sea Foods (Ph: 9441429745)`;
